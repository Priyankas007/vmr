...
class SurgicalStepDataset(Dataset):
    """
    Dataset for surgical step recognition (THIS IS IMPLMEMENTED TO MATCH GSVIT PRETRAINING PIPEPLINE)
    """    
    ...
    def __getitem__(self, idx):
        ...
        # resize shorter side to 224 and center crop (THIS IS IMPLMEMENTED TO MATCH GSVIT PRETRAINING PIPEPLINE)
        h, w = image.shape[:2]
        scale = 224 / min(h, w)
...
def create_data_loader(data_root, split = 'train', batch_size = 128, num_workers = 4):

    assert split in ['train', 'val', 'test']
    
    
    dataset = SurgicalStepDataset(root_dir = data_root, split = split)
    print(f"Creating data set for {split} split")
...
def visualize_batch(batch, save_path='batch_visualization.png'):
    """
    ASSUMES BATCH IS A TUPLE OF (IMAGES, LABELS) FROM DATALOADER AND BATCH_SIZE = 4 !!
    """
    images, labels = batch
